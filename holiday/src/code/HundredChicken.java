...
//	   用100 文钱买100 只鸡，那么各有公鸡、母鸡、小鸡多少只
    public static void main(String[] args) { // 本题使用穷举法
        for (int roosterNumber = 1; roosterNumber <= 33; roosterNumber++) { // 如果100文全部买公鸡最多可以买33只
            for (int henNumber = 1; henNumber <= 50; henNumber++) { // 如果100文全部买母鸡最多可以买50只
                for (int chickenNumber = 3; chickenNumber <= 100; chickenNumber += 3) { // 一百文最多可以买300只小鸡,但根据题意,鸡的数量最多为100,故取值100,又因三只小鸡一文钱,故将3只小鸡作为一个单位,小鸡的数量从3开始,+=3
